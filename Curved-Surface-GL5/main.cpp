...
const float playerHight = 3.0; // это не совсем рост игрока, скорее высота камеры, но чем выше тем хуже видно поверхности.
double timeValue = glfwGetTime();
//эксперимент со структурами
typedef struct {
    float x, y, z;
} Cell;
...
//вращаться или не вращаться, вот в чём вопрос...
...
//эта штука должна была делать смену дня и ночи, но она - ломает пк к чертям
/*float colorOfDay(){
    if (DayColor == 1.0f){DayColor -= 0.00001f;}
    else {DayColor += 0.00001f;}
}*/

//отрисовка гор и их разрисовка
        ...
        for (int j = centerY - radius; j <= centerY + radius; j++) {
            if (onMap(i, j)) {
                ...
                if (distance < radius) {
                    ...
                    mapColors[i][j].r = 1/0.9 - gradientX;
                    mapColors[i][j].g = 1/0.8 - gradientY;
                    mapColors[i][j].b = 0.3;
                }
            }
        }
...
void initializeMap() {
    for (int i = 0; i < MAP_WIDTH; i++) {
        for (int j = 0; j < MAP_HEIGHT; j++) {
            float colorOffset = (rand() % 10) * 0.01;
            mapColors[i][j].r = 0.1 + colorOffset;
            mapColors[i][j].g = 0.1 + colorOffset;
            mapColors[i][j].b = 0.1 + colorOffset;
...
void moveCamera(GLFWwindow* window) {
    ...
    if (onMap(newX, newY)) {
        ...
        tmpZ = getHeight(cameraPosition[0], cameraPosition[1]) + playerHight;
    }
...
int main() {
    if (!glfwInit()) {
        std::cerr << "GLFW is De@d" << std::endl;
        exit(1);
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glClearColor(0.3f, 0.3f, 0.3f, 1.00f); // фоновый цвет мира (около серый)
